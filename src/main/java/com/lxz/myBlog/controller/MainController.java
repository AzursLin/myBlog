...
public class MainController {
    ...
    public String index(){
        Gson gs = new Gson();
        try {
            File xlsFile = new File("C://批一蓝牙导入.xls");
            WritableWorkbook writeBook = Workbook.createWorkbook(xlsFile);
            WritableSheet firstSheet = writeBook.createSheet("批一蓝牙导入", 1);
            ...
            String strH,strL;
            int hNum,lNum;
            int nameStarNumLen = 400; //长度
            int nameStarNum = 101; //名字起始
            int nowNum = 101;  //mac起始地址


            for (int i=0;i<nameStarNumLen;i++) {
                firstSheet.addCell(new Label(0, i+1,  String.format("%06d", nameStarNum+i)));    //蓝牙名

                hNum = (nowNum+i)/256;
                lNum = (nowNum+i)%256;

                if (hNum < 16) {
                    strH = "0"+Integer.toHexString(hNum);
                }
                else {
                ...
                if (lNum < 16) {
                    strL = "0"+Integer.toHexString(lNum);
                }
                else {
                    strL = Integer.toHexString(lNum);
                }
                firstSheet.addCell(new Label(1, i+1,  " 00:DB:00:00:"+strH+":"+strL));
                firstSheet.addCell(new Label(2, i+1,  "1"));
                firstSheet.addCell(new Label(3, i+1,  "动感单车"));
